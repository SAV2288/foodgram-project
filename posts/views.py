        ...
        if recipe_form.is_valid() and tag_form.is_valid():
            ...
            if ingredients:
                ...
                for ingredient in ingredients:
                    Recipe_composition.objects.create(ingredient = ingredient["obj"], amount=ingredient["amount"], recipe=recipe)
                
                tags = function.get_tags(tag_form.cleaned_data)
                for tag in tags:
                    tag_recipe = Recipe_tag.objects.create(tag=tag, recipe=recipe)
...
def post_edit(request, username, post_id):
    ...
    recipe_form = NewRecipe(request.POST or None, files=request.FILES or None, instance=recipe)
    recipe_comp_form = NewRecipeComp(request.POST or None)
    tag_form = TagForm(request.POST or None)
    ...
    if request.method == "POST":
        if recipe_form.is_valid() and tag_form.is_valid():
            ...
            if ingredients:
                ...
                return redirect("/")   # Выход, если редактирование прошло успешно
...
def favorites_index(request):
    ...
    recipe_list = Favorites.objects.filter(user=request.user).values_list("recipe")
