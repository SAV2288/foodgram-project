...
def profile_follow(request, username):
    ...
    if request.user != author:
        ...
        if follow[1]:
            result = {
                "status": "true",
                "message": f"Вы подписались на автора \
...
def profile_unfollow(request, username):
    ...
    return JsonResponse(
        {
            "status": "true",
            "message": f"Автор {author.first_name} {author.last_name} \
                удален из подписки"
        },
        status=200,
        safe=False
...
def recipe_favorite(request, recipe_id):
    ...
    if request.user != recipe.author:
        ...
        result = {
            "status": "false",
            "message": "Нельзя добавить в избранное свой рецепт"
        }
        status = 400
    return JsonResponse(result, status=status, safe=False)
...
def get_ingredients(request, name):
    ...
    ingreduents = Ingredient.objects.filter(name__startswith=name)\
        .values("name", "units")
    results = [{"name": ingredient["name"], "units":ingredient["units"]}
...
def recipe_to_shop_list(request, recipe_id):
    ...
    recipe = get_object_or_404(Recipe, id=recipe_id)
    add_to_shop_list = Shop_list.objects.get_or_create(
        user=request.user,
        recipe=recipe
    )
    if add_to_shop_list[1]:
        result = {
            "status": "true",
            "message": f"Рецепт '{recipe.title}' добавлен в список покупок"
        }
        status = 200
    else:
        result = {
...
def recipe_remove_in_shop_list(request, recipe_id):
    ...
    recipe = get_object_or_404(Recipe, id=recipe_id)
    Shop_list.objects.filter(user=request.user, recipe=recipe).delete()
    return JsonResponse(
        {
            "status": "true",
            "message": f"Рецепт '{recipe.title}' удален из списока покупок"
        },
        status=200,
        safe=False
    )
...
def download_shop_list(request):
    ...
    try:
        with open(
            f"temp_file/Shop_list({ request.user.username }).txt", "w"
        ) as f:
            f.write("FoodGram | Список покупок\n\n")
            f.writelines(ingredients)
        response = FileResponse(open(
                f"temp_file/Shop_list({ request.user.username }).txt", "rb"
            ))
        return response
    finally:
